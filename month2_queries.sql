-- String functions for investigation
SELECT user_id,
	query_text,
	LENGTH(query_text) AS query_length,
	LOWER(query_text) AS query_lower,
	INSTR(LOWER(query_text), 'compound') AS compound_position,
	CASE WHEN INSTR(LOWER(query_text), 'compound') > 0 THEN 'Yes'
		ELSE 'No'
	END AS contains_compound
FROM capstone_logs
LIMIT 15;
...
SELECT user_id,
	...
	CASE
		WHEN INSTR(LOWER(query_text), 'synthesize') > 0 
           ...
           ELSE 'Benign'
	END AS cbrn_catagory
FROM capstone_logs
LIMIT 20;

-- Pivot table -- CBRN catagory counts per user
WITH classified AS (
	SELECT user_id,
		CASE 
               WHEN INSTR(LOWER(query_text), 'synthesize') > 0 
...
SELECT user_id,
	...
	SUM(CASE WHEN category ='Biological' THEN 1 ELSE 0 END) AS biological,
	SUM(CASE WHEN category = 'Radiological' THEN 1 ELSE 0 END) AS radiological,
	SUM(CASE WHEN category = 'Explosive' THEN 1 ELSE 0 END) AS explosive,
	SUM(CASE WHEN category = 'Benign' THEN 1 ELSE 0 END) as benign,
	COUNT(*) AS total
FROM classified
GROUP BY user_id
ORDER BY (chemical + biological + radiological + explosive) DESC;
